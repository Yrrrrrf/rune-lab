...
const typeDetails: Record<
  string,
  { alertColor: string; iconColor: string; iconPath: string }
> = {
  info: {
...
>
  {#each toastStore.toasts as toast (toast.id)}
    ...
    <div
      class="toast-item alert {styles.alertColor} alert-soft pointer-events-auto relative flex w-full max-w-sm items-start gap-4 overflow-hidden rounded-xl p-4 shadow-lg backdrop-blur-xl transition-all duration-300 sm:min-w-[320px] animate-in fade-in slide-in-from-bottom-4 zoom-in-95 ease-out data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=closed]:slide-out-to-right-full"
      role="alert"

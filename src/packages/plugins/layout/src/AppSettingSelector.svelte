...
<script lang="ts" generics="T">
  ...
  function computePosition() {
    ...
    const openUpward = direction === "top" ||
      (direction === "auto" &&
        spaceBelow < 200 &&
        spaceAbove > spaceBelow);

    if (openUpward) {
      panelStyle = `position:fixed; bottom:${
        window.innerHeight - rect.top + 4
      }px; left:${rect.left}px; min-width:${rect.width}px; z-index:9999`;
    } else {
      panelStyle = `position:fixed; top:${
        rect.bottom + 4
      }px; left:${rect.left}px; min-width:${rect.width}px; z-index:9999`;
    }
  }
...
>
  <button
    ...
    onclick={open}
    class="
      btn btn-ghost btn-sm m-1 {tooltip

...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 p-4">
  ...
  <ShowcaseCard title="Modal & Tooltip" icon="🪟">
    ...
    <button class="btn btn-primary" onclick={() => (document.getElementById('demo_modal') as HTMLDialogElement).showModal()}>
      Open Modal
    </button>
  ...
  <ShowcaseCard title="Loading & Progress" icon="⏳">
    ...
    <div class="space-y-4">
      <progress class="progress progress-primary w-full" value={progressVal} max="100"></progress>
      <div class="flex flex-wrap gap-4 justify-center">
         <div class="radial-progress text-primary" style="--value:{progressVal};" role="progressbar">
           {progressVal}%
         </div>
      </div>
      <input type="range" min="0" max="100" bind:value={progressVal} class="range range-xs" />
    </div>
  </ShowcaseCard>

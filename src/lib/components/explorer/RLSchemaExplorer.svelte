...
<script lang="ts">
    import type {
        ...
        ViewMetadata,
        EnumMetadata,
        FunctionMetadata, // Covers Functions, Procedures, Triggers
        ColumnMetadata,
        // APIOperation type if defined globally, or define locally
    ...
    let modalTargetColumns = $state<ColumnMetadata[]>([]); // For tables/views
    let modalTargetOperation = $state<APIOperation>('GET');
    // For functions, we might pass the FunctionMetadata itself or its parameters
    ...
    $effect(() => { // Effect to update currentSelectedSchemaObject
        if (!activeSchemaName || !schemas) currentSelectedSchemaObject = null;
        else currentSelectedSchemaObject = schemas.find(s => s.name === activeSchemaName) || null;
    });

    let currentEntityItems = $state<Record<string, any> | null>(null);
    $effect(() => { // Effect to update currentEntityItems
    ...
    $effect(() => { // Load Schemas (remains the same)
        async function loadSchemas() {
            if (apiStore.IS_CONNECTED && apiStore.prism) {
                ...
                isLoading = true; error = "API not connected. Waiting for connection...";
                schemas = null;
            }
        }
        loadSchemas();
    ...
    function getEntityCount(schema: SchemaMetadata | null, entityType: EntityType): number {
        if (!schema) return 0;
        const entities = schema[entityType]; // Accessing e.g., schema.tables, schema.views
    ...
    function getColumnsForTableOrView(item: TableMetadata | ViewMetadata): ColumnMetadata[] {
        return item.columns || []; // Both have a .columns property in prism-ts types
    }
    ...
    function openModalForOperation(
        ...
        currentResourceType: 'table' | 'view' | 'function', // Added 'function'
        currentColumnsOrParams: ColumnMetadata[] | FunctionMetadata['parameters'] // Union type
    ) {
        ...
        if (currentResourceType === 'table' || currentResourceType === 'view') {
            modalTargetColumns = currentColumnsOrParams as ColumnMetadata[];
            modalTargetFunctionParams = null;
        } else if (currentResourceType === 'function') { // For functions, procedures, triggers
            modalTargetColumns = []; // Functions don't have "columns" in the same way for forms
            modalTargetFunctionParams = currentColumnsOrParams as FunctionMetadata['parameters'];
        }
        isModalOpen = true;
    }
    ...
    {:else if schemas && schemas.length > 0}
        <div role="tablist" class="tabs tabs-lifted tabs-lg mb-6">
            {#each schemas as schema}
                <button role="tab" class="tab {activeSchemaName === schema.name ? 'tab-active font-semibold' : ''}" onclick={() => activeSchemaName = schema.name} aria-selected={activeSchemaName === schema.name}>
                    {schema.name}
                </button>
            {/each}
        </div>
            ...
            <div class="bg-base-200 p-4 rounded-box">
                <div role="tablist" class="tabs tabs-bordered mb-6">
                    {#each ['tables', 'views', 'enums', 'functions', 'procedures', 'triggers'] as typeStr}
                        ...
                        <button role="tab" class="tab {activeEntityType === entityType ? 'tab-active' : ''} [--tab-bg:oklch(var(--b2))] [--tab-border-color:oklch(var(--b3))]" onclick={() => activeEntityType = entityType} aria-selected={activeEntityType === entityType} disabled={count === 0}>
                ...
                <div class="space-y-6">
                            ...
                            {#each Object.entries(currentEntityItems) as [name, itemData]}
                                ...
                                <div class="collapse collapse-arrow bg-base-100 shadow-md">
                                    ...
                                    <div class="collapse-content">
                                        <RLMetadataTable title={name} itemType={currentItemType} columns={itemColumns} />
                                        <div class="mt-4 p-2 border-t border-base-300">
                                            <RLApiInterface
                                                ...
                                                columns={itemColumns}
                                                onOpenModal={(params) => openModalForOperation(params, activeSchema.name, name, currentItemType, itemColumns)}
                                            />
                                        </div>
                                    </div>
                                </div>
                            {/each}
                        {:else} <p class="text-center p-4 text-neutral-content/70">No {activeEntityType} found in this schema.</p> {/if}
                    {:else if activeEntityType === 'enums'}
                        {#if currentEntityItems && Object.keys(currentEntityItems).length > 0}
                            ...
                            {#each Object.entries(currentEntityItems) as [name, enumData]}
                                ...
                                <div class="card bg-base-100 shadow-md">
                                    <div class="card-body">
                                        <h4 class="card-title text-base">{name} <span class="badge badge-outline badge-sm">enum</span></h4>
                        ...
                        {:else} <p class="text-center p-4 text-neutral-content/70">No enums found in this schema.</p> {/if}
                            ...
                            {#each Object.entries(currentEntityItems) as [name, funcData]}
                                ...
                                <div class="card bg-base-100 shadow-md">
                                    <div class="card-body">
                                        ...
                                        {#if typedItem.description} <p class="text-xs text-neutral-content/70 italic mt-1 mb-2">{typedItem.description}</p> {/if}
                                                ...
                                                <table class="table table-xs">
                                                    ...
                                                    <tbody>
                                                        {#each typedItem.parameters as param}
                                                            <tr><td>{param.name}</td><td class="font-mono">{param.type}</td><td><span class="badge badge-neutral badge-outline badge-xs">{param.mode}</span></td><td>{param.hasDefault ? '✓' : '✗'}</td></tr>
                                                        {/each}
                                                    </tbody>
                                                </table>
                                            </div>
                                        ...
                                        {#if typedItem.returnType && typedItem.returnType !== 'void'}
                                            <div class="mt-2"><span class="font-semibold text-sm">Returns:</span><span class="font-mono text-xs ml-2 badge badge-outline">{typedItem.returnType}</span></div>
                                        {/if}
                                        <div class="mt-4 p-2 border-t border-base-300">
                                             <!-- RLApiInterface for functions/procs -->
                                             <RLApiInterface
                                                schemaName={activeSchema.name}
                                                resourceName={name}
                                                ...
                                                onOpenModal={(params) => openModalForOperation(
                                                    ...
                                                    activeSchema.name,
                                                    name,
                                                    'function', // Treat all as 'function' for API call type
                                                    typedItem.parameters // Pass parameters for form generation
                                                )}
                                            />
                                        </div>
                                    </div>
        ...
        {:else if !isLoading} <div role="alert" class="alert alert-info"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="stroke-current shrink-0 w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><span>Please select a valid schema.</span></div> {/if}
    {:else if !isLoading && (!schemas || schemas.length === 0)} <div role="alert" class="alert"><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" class="stroke-info shrink-0 w-6 h-6"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg><span>No schemas found.</span></div> {/if}
